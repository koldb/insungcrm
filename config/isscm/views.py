...
def index(request):
    ...
    es_fcount = main_sheet.objects.filter(rg_date__gte=date.today(),
                                          finish="종료").count()
    es_pfcount = main_sheet.objects.filter(rg_date__gte=date.today(),
    ...
    as_pcount = ASsheet.objects.filter(rg_date__gte=date.today(),
                                       cname=login_session).count()
    as_fcount = ASsheet.objects.filter(rg_date__gte=date.today(), finish="종료").count()
    as_pfcount = ASsheet.objects.filter(rg_date__gte=date.today(), finish="종료", cname=login_session).count()
    as_icount = ASsheet.objects.filter(rg_date__gte=date.today() - relativedelta(months=1), finish="진행 중").count()
    as_picount = ASsheet.objects.filter(rg_date__gte=date.today() - relativedelta(months=1), finish="진행 중", cname=login_session).count()
    as_xcount = ASsheet.objects.filter(rg_date__gte=date.today() - relativedelta(months=1), finish="").count()
    as_pxcount = ASsheet.objects.filter(rg_date__gte=date.today() - relativedelta(months=1), finish="", cname=login_session).count()
    ...
    context = {'login_session': login_session, 'es_count': es_count, 'as_count': as_count,
               ...
               'es_cnum': es_cnum, 'as_cnum': as_cnum, 'es_fcount': es_fcount, 'as_xcount': as_xcount, 'as_icount': as_icount,
               ...
               'as_pfcount': as_pfcount, 'as_picount': as_picount, 'as_pxcount': as_pxcount}
        ...
        if login_session == 'insung':
            ...
            if sort == 'rg_date':
                ...
                if search_sort == 'main_title':
                    ...
                    s_sheet = sub_sheet.objects.all().filter(product_name__icontains=query, cname=login_session).order_by('-rg_date')
                    v = []
                    for i in s_sheet:
                        sm = i.m_id_id
                        ms = main_sheet.objects.filter(id__icontains=sm)
...
def sub_delete(request, pk, mid):
    ...
    detailView = get_object_or_404(main_sheet, id=mid)
    sub = sub_sheet.objects.all().filter(m_id_id=mid).order_by('-release_date')
        ...
        if login_session == 'insung':
            ...
            if sort == 'rg_date':
                ...
                if search_sort == 'product_name':
                    ...
                    sub_list = sub_sheet.objects.all().filter(serial__icontains=query, cname=login_session).order_by('-serial',
                                                                                                                     '-rg_date')
                else:
                    sub_list = sub_sheet.objects.filter(cname=login_session).order_by('-rg_date', 'id')
    # 페이징
