...
def sheet_list(request):
    print("리스트 시작")
    login_session = request.session.get('login_session')
    fileck = UploadFile.objects.filter(sheet_no=EstimateSheet.no)
    if fileck:
        fileck = models.EstimateSheet.fileck(
            fileck= '파일 있음',
        )
        fileck.save()
    ...
    if request.method == 'GET':
        ...
        page = pagination(request, company_sheet)
        print(page)
        context = {'login_session': login_session, 'page': page, 'company_sheet': company_sheet, 'filepk': filepk}
        print('끝')
